...
| 特定機能のVFを新規タブで開いた瞬間に失敗 | コミュニティ経由のVF配信がCDN層で失敗（例: DocuSign DAL `dfsle__embeddedsigning` 埋め込み署名）。VFアクセス権があっても発生 | そのVFのサイト配信は別途サイト準備が必要。代替の非VF経路へ切替（DocuSignなら内部Send→メール署名）。`docusign-dal-esignature.md` §6 |
