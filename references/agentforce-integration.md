...
#### ⚠️ ADL自動生成「コード」種別 Retriever は Prompt Template から直接呼べない（重要・実証済み）

ADL作成時に自動生成される Retriever は **「コード」種別**（Library 内部処理用、grounding source ID によるプリフィルタが必須の構造）として作られる。これを Prompt Template の `templateDataProviders` として参照すると、**ヒット 0 件で返ってくる**（エラーは出ないため発覚が遅れる典型的な罠）。

**対処**: Prompt Template から RAG を呼ぶ場合は、**「個別」種別 Retriever** を chunk DMO ベースで**手動作成**して使う：

1. Setup → Einstein → Einstein Studio → Retrievers → **New → 個別レトリーバー**
2. データソース: **Data Cloud**
3. Data Model Object: ADL が生成した chunk DMO（`<LibraryName>_chunk__dlm` 形式）を選択
4. Search Index: ADL が生成した Search Index を選択
5. 返却フィールド・チャンク数を設定 → Save → **Activate**
6. この個別 Retriever を Prompt Template の `templateDataProviders.definition` で参照

ADL 自動生成 Retriever はあくまで Library の `Answer Questions with Knowledge` アクション**専用**と理解する。Prompt Template 経由で利用したい場合は必ず個別 Retriever を別途作成すること。
...
7. **返却フィールドの設定（重要）**:
   ...
   **Knowledge__kav 由来 DMO の場合の注意（実証済み）**: Knowledge Article Version (`Knowledge__kav`) を CRM Connector で取り込むと、標準項目 `Title` は ssot DMO 上で **`ssot__Name__c`** にマッピングされる。Retriever UI の項目リストに `Title` は出ないので **「Name」を選ぶ**（混乱しがちなので注意）。
