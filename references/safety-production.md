...
- 5. 緊急時のロールバック手順
- 6. 禁止事項（本番環境）
- 7. 外部 LLM Agent（Slack / Claude 等）から SF を操作する場合のガードレール
...
スキル本体は Salesforce CLI ベースだが、外部 LLM Agent（Slack エージェント、Claude エージェント等）が **SF MCP / REST API 経由で書き込み操作を行うシステムを構築する場合**、システムプロンプトで書込対象を明示的に制約する必要がある。曖昧なルールは OCR / 文書抽出パイプライン等が予期せず書込を試みて巻き添えで誤書込を発生させる。

### 7-1. ホワイトリスト方式を推奨

書込許可対象は **カスタムオブジェクト限定のホワイトリスト** で列挙する：

```
✅ 推奨: 書込可能なオブジェクトを明示列挙
書込可: Needs_Card__c, Meeting_Record__c, Opportunity_Summary__c
書込不可: それ以外すべて（標準オブジェクト含む）

❌ 非推奨: 書込不可だけを列挙
書込不可: Account, Contact, Opportunity
書込可: それ以外  ← LLMが解釈を広げて意図しないオブジェクトを書く
```

### 7-2. 標準オブジェクトを書込許可する場合は範囲を明示

標準オブジェクト（Account, Contact, Task 等）も書込許可したい場合は、**「従来通り書込 OK」と明示**する。スコープが暗黙だと巻き添え誤書込が発生する。

### 7-3. システムプロンプト記述例

```
## Salesforce 書込ルール
- 以下のカスタムオブジェクトのみ書込可（ホワイトリスト）:
  - Needs_Card__c (面談で抽出したニーズ)
  - Meeting_Record__c (議事録)
  - Opportunity_Summary__c (商談サマリ)
- 標準オブジェクト（Account, Contact, Opportunity 等）は従来通り書込可
- 上記以外のカスタムオブジェクトは読み取り専用。書込を要求された場合はユーザーに確認を求める
```
