...
このスキルは実践で得た知見を蓄積して改善していくことを前提としている。

- **発見時の提案**: Salesforce実装中に新たなエラー・制約・ワークアラウンドを発見した場合、一般的に再発しうる知見であれば `references/` 配下の該当ファイルへの反映を**ユーザーに提案する**
- **勝手に更新しない**: ユーザーの承認を得てからスキルファイルを更新する
- **環境固有の問題は書かない**: 特定のorg・デモ環境でのみ発生する事象はスキルに含めない。複数環境で再現しうる一般的な知見のみ対象とする
- **検証済みの知見のみ**: 1回の試行で得た推測ではなく、原因と対処が確認できた事象を記録する
