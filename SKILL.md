...
description: "Claude Code専用のSalesforce管理スキル。データの登録・更新（単件/バルク）、カスタムオブジェクト・フィールド・レイアウト・権限セット・Apex・フローなどメタデータ操作をCLIで実行。本番環境対応。"
---

# salesforce-admin Skill

## Description
Claude Code専用のSalesforce管理スキル。データの登録・更新（単件/バルク）、カスタムオブジェクト・フィールド・レイアウト・権限セット・Apex・フローなどメタデータ操作をCLIで実行。本番環境対応。
