...
**Salesforce Hosted MCP 経由で運用する場合の注意**:
Hosted MCP が発行する `access_token` は **`mcp_api` scope のみ**で、MCP tool 経由の呼出ししかできない。
直接 REST API を叩く（`sf api request rest` 経由含む）必要がある場合は、別途 **JWT Bearer Flow** で `api` scope の access_token を取得する。Hosted MCP の token をそのまま REST に渡すと 401 になる。
...
| `INVALID_TYPE: SlackChannelRelatedRecord` | REST API バージョンが古い。**v66.0 以上で呼び出す**必要あり（v60-v62 では使用不可） |
